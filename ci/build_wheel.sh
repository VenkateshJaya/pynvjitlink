...
python -m auditwheel repair --exclude libcuda.so.1 -w "${RAPIDS_WHEEL_BLD_OUTPUT_DIR}" ./dist/*

rapids-logger "Upload Wheel"
RAPIDS_PY_WHEEL_NAME="pynvjitlink_${RAPIDS_PY_CUDA_SUFFIX}" rapids-upload-wheels-to-s3 python "${RAPIDS_WHEEL_BLD_OUTPUT_DIR}"
